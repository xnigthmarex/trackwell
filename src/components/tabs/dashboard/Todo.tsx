...
export default function Todo(props: any) {
    ...
    <div className="mt-4 flex items-center justify-start lg:justify-center">
        <input 
            type="text" 
            placeholder="Enter your task..." 
            className="px-4 py-2 mr-2 border-2 border-purple-500 rounded bg-inherit font-extrabold placeholder-gray-200 flash-purple w-[60%]" 
            ref={inputRef} 
        />
        <button 
            className="px-4 py-2 border-2 font-extrabold text-white rounded glow-text border-green-700 flash-green flex items-center justify-center" 
            onClick={addTodo} 
            disabled={addingTodo}
        >
            {addingTodo ? (
                <ThreeDots className="w-6 h-[1.3rem] text-white glow-text" />
            ) : (
                'Add'
            )}
        </button>
    ...
    <div className="mt-4 mx-16">
        {loading ? (
            <SpinningCircles className="w-20 h-20 text-white glow-text mx-auto" />
        ) : (
            todos.map((todo: Todo) => (
                <div 
                    key={todo.id} 
                ...
                >
                    ...
                    <button 
                        className="px-2 py-1 ml-2 bg-red-500 text-white rounded hover:bg-red-600 flex items-center justify-center" 
